...
echo "============================================="
echo "  Musl 静态编译 - armv7-unknown-linux-musleabihf"
echo "============================================="
...
# musl CRT 文件路径（crt1.o, crti.o, crtn.o, libc.a）
MUSL_LIB_DIR="$TOOLCHAIN_DIR/${CROSS_PREFIX}/lib"
# GCC CRT 文件路径（crtbegin.o, crtend.o）
GCC_VERSION=$($CROSS_GCC -dumpversion)
GCC_LIB_DIR="$TOOLCHAIN_DIR/lib/gcc/${CROSS_PREFIX}/${GCC_VERSION}"

echo "CC: $CROSS_GCC"
echo "GCC version: $($CROSS_GCC --version | head -1)"
echo "MUSL lib: $MUSL_LIB_DIR"
echo "GCC lib:  $GCC_LIB_DIR"
...
export CC_armv7_unknown_linux_musleabihf="$CROSS_GCC"
export CXX_armv7_unknown_linux_musleabihf="$CROSS_CXX"
export AR_armv7_unknown_linux_musleabihf="$CROSS_AR"

# Cargo linker
export CARGO_TARGET_ARMV7_UNKNOWN_LINUX_MUSLEABIHF_LINKER="$CROSS_GCC"

# 完全静态链接
# -lgcc:    提供 ARM EABI unwind 符号（__aeabi_unwind_cpp_pr0）和原子操作内建函数（__sync_*）
# -lgcc_eh: 提供 unwind 符号（_Unwind_Resume 等），Rust 预编译 std 的 backtrace 功能需要
export RUSTFLAGS="-C target-feature=+crt-static -C link-arg=-lgcc -C link-arg=-lgcc_eh"
